...
const VoxelScene: React.FC<VoxelSceneProps> = ({ level, small = false, isCompleted = false }) => {
  ...
  // Animation ref to keep track of rotation without state lag
  ...
  useEffect(() => {
    ...
    setRotation({ x: 60, z: 45 });
  }, [level]);

  // Handle Level Completion Effect
  useEffect(() => {
    if (isCompleted) {
      // Staggered dissolve of accent blocks for a "cleansing" effect
      const targetBlocks: string[] = [];
      gridPattern.forEach((row, y) => {
        row.forEach((code, x) => {
          // Dissolve Accent (2) and Path (1) blocks for full effect
          if (code === 2 || code === 1) targetBlocks.push(`${x}-${y}`);
        });
      });

      // Sort roughly by distance from center for a ripple effect
      ...
      if (targetBlocks.length > 0) {
        let i = 0;
        // Faster interval for a satisfying ripple
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (!isHovering) {
        ...
        setRotation(prev => ({ ...prev, z: rotationRef.current }));
      }
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    // Interactive tilt limits
    const rotateX = Math.max(20, Math.min(80, 60 - (y / rect.height) * 40)); 
    const rotateZ = rotationRef.current + (x / rect.width) * 40;
    
    setRotation({ x: rotateX, z: rotateZ });
  };
  ...
  return (
    ...
    >
      ...
      {/* 3D Container */}
      <div 
        className="relative preserve-3d transition-transform duration-100 ease-linear will-change-transform"
        style={{ 
      ...
      >
        ...
        {gridPattern.map((row, y) => 
          row.map((code, x) => {
            ...
            if (isDissolved) {
               ...
               // We use a positioning wrapper to handle X/Y/StartZ
               // Inner div handles the animation relative to that start
               return (
                 <div 
                    key={`${x}-${y}`}
        ...
        {!small && (
           <div 
             className="absolute preserve-3d" 
             style={{ 
               // Base position handled in CSS keyframes for smoother idle animation
               // Center of grid is effectively 0,0 now because of our offset logic
               left: '50%', top: '50%',
